...
_alert_queue = asyncio.Queue()  # 新的提醒队列（生产者-消费者模型）
...
async def _alert_worker(worker_id: int):
    ...
    while True:
        try:
            ...
            bot_client, account, event, matched_keyword, control_bot_id = task_data
            
            # 在这里才进行耗时的网络请求（get_sender, get_chat）
            # 即使这里卡住 1秒，也不会漏掉新的消息监听
            try:
                # 并发获取 sender 和 chat 信息
                ...
                # 调用原有的 send_alert 逻辑（但使用已获取的 sender 和 chat）
                await send_alert_with_info(bot_client, account, event, matched_keyword, sender, chat, control_bot_id)
            except Exception as e:
                print(f"[提醒工作协程 #{worker_id}] ❌ 处理失败: {e}")
            
            _alert_queue.task_done()
        except Exception as e:
            print(f"[提醒工作协程 #{worker_id}] ❌ 错误: {e}")
...
def _ensure_alert_workers(bot_client):
    ...
    if not _alert_workers_started:
        # 启动多个提醒工作协程
        num_workers = 100  # 100个并发提醒工作协程
        _alert_workers = [asyncio.create_task(_alert_worker(i)) for i in range(num_workers)]
        _alert_workers_started = True
        print(f"[提醒队列] 启动 {num_workers} 个提醒工作协程，确保真正并发")


def quick_enqueue(bot_client, account, event, matched_keyword: str, control_bot_id=None):
    """
    极速入队：立即将提醒任务放入队列，不阻塞主线程
    这是生产者-消费者模型的生产者端
    """
    _ensure_alert_workers(bot_client)
    try:
        _alert_queue.put_nowait((bot_client, account, event, matched_keyword, control_bot_id))
    except Exception as e:
        print(f"[极速入队] ❌ 入队失败: {e}")


async def send_alert_with_info(bot_client, account, event, matched_keyword: str, sender, chat, control_bot_id=None):
    """发送提醒（使用已获取的 sender 和 chat 信息，避免重复网络请求）"""
    from datetime import datetime
    ...
    if not target or not target.strip():
        ...
        try:
            ...
            if target_clean.startswith('https://t.me/+') or target_clean.startswith('https://t.me/joinchat/') or target_clean.startswith('t.me/+') or target_clean.startswith('t.me/joinchat/'):
                raise ValueError('转发目标不能是邀请链接。机器人无法解析邀请链接。')
